...
from .constants import WEIGHTS, CRITICALITY_WEIGHTS
...
class BnafarAnalytics:
    ...
    def calculate_confidence_score(df: pd.DataFrame) -> pd.DataFrame:
        ...
        score = (temporal_freq * WEIGHTS['temporal_consistency']) + \
                (network_coverage * WEIGHTS['network_coverage']) + \
                (recency_score * WEIGHTS['recency'])

        muni_info = df[['co_municipio_ibge', 'no_municipio', 'sg_uf']].drop_duplicates('co_municipio_ibge').set_index('co_municipio_ibge')
    ...
    def calculate_priority_waste(df: pd.DataFrame, days: int = 90) -> pd.DataFrame:
        ...
        }).sort_values('severity_index', ascending=False).reset_index()
