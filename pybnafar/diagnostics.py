...
class BnafarDiagnostics:
    ...
    def check_geographic_bias(df: pd.DataFrame):
        ...
        logger.warning("ETHICAL NOTICE: Low confidence scores often reflect infrastructure gaps (lack of internet/staff), not necessarily poor management. Do not use these scores to justify resource withdrawal.")
        
        for uf, count in coverage.items():
            pct = (count / total_muni) * 100
            if pct < 2.0: 
                logger.warning(f"Potential Bias: State {uf} represents only {pct:.2f}% of the municipalities.")
    ...
    def validate_integrity(df: pd.DataFrame):
        ...
        if not outliers.empty:
            logger.warning(f"Integrity Alert: {len(outliers)} records found with extreme volumes (>1M units).")
        return outliers
